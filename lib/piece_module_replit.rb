...
class BoardSquare
  def self.for(notation, previous_color, selected = nil, destination = nil)
    choose_selected_square(notation) unless selected.nil?
    choose_destination_square(notation) unless destination.nil?
...
class Square < BoardSquare
  ...
  def to_s
    "#{GREEN_SQUARE} \u001b[32m\u25CB#{GREEN_SQUARE} #{ESCAPE_CODE}"
  end
end
...
class WhiteSquare < BoardSquare
  ...
  def to_s
    "#{WHITE_SQUARE} \u001b[37m\u25CB#{WHITE_SQUARE} #{ESCAPE_CODE}"
  end
end
